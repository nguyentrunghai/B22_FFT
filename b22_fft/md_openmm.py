...
class OpenMM_MD(object):
    def __init__(self, prmtop_file, inpcrd, phase="OpenMM_Gas", temperature=300.):
        """
        :param prmtop: str, name of prmtop file
        :param inpcrd: str, name of inpcrd file
        :param phase: str, phase name
        :param temperature: float
        """
        self._prmtop_file = prmtop_file
        self._prmtop = simtk.openmm.app.AmberPrmtopFile(prmtop_file)
        ...
        integrator = simtk.openmm.LangevinIntegrator(temperature*simtk.unit.kelvin, 1/simtk.unit.picosecond,
    ...
    def _initialize_nc(self, nc_file_name, niterations):
        nc_handle = nc.Dataset(nc_file_name, mode="w", format="NETCDF4")
    ...
    def run(self, nc_file_name, steps_per_iteration=500, niterations=1000,
            inpcrd_prefix=None, pdb_out=None):
        """
        :param nc_file_name: str, nc file to store trajectory
        :param steps_per_iteration: int
        :param niterations: int
        :param inpcrd_prefix: None or str
        :param pdb_out: None or str
        :return: None
        """
...
class OpenMM_TREMD(object):
    ...
    def _create_simulation(self, prmtop, inpcrd, phase, temperature):
        ...
        system = prmtop.createSystem(nonbondedMethod=simtk.openmm.app.NoCutoff, constraints=simtk.openmm.app.HBonds,
                                     implicitSolvent=selected_solvent)
        integrator = simtk.openmm.LangevinIntegrator(temperature*simtk.unit.kelvin, 1/simtk.unit.picosecond,
                                                     0.002*simtk.unit.picoseconds)
